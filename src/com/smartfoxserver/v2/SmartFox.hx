...
extern class SmartFox //implements Dynamic
{
	...
	/////
	public var joinedRooms(get, null):Array<Dynamic>;
	inline function get_joinedRooms():Array<Dynamic>
	{
		return cast getJoinedRooms();
	}
...
//	inline function connect(?host:String, ?port:Int, ?useSSL:Bool):Void
//	{
//
//		data.currentPort = port;
//		//Reflect.setField(this, "currentPort", port);
//		data.currentIp = host;
//		data.useSSL = useSSL;
...
{	
	...
	*/
	...
	{
		reset();
		dispatchEvent(SFSEvent.newInst(SFSEvent.CONNECTION_LOST, { reason:evt.params.reason } ));
	}
	
	private function onSocketReconnectionTry(evt:BitSwarmEvent):Void
	{
		dispatchEvent(SFSEvent.newInst(SFSEvent.CONNECTION_RETRY, { } ));
	}
	...
	private function onConfigLoadSuccess(evt:SFSEvent):Void
	{
		...
		// Fire event
		var sfsEvt:SFSEvent = SFSEvent.newInst(SFSEvent.CONFIG_LOAD_SUCCESS, { config:cfgData } );
	...
	private function onConfigLoadFailure(evt:SFSEvent):Void
	{
		...
		// Fire event;
		var sfsEvt:SFSEvent = SFSEvent.newInst(SFSEvent.CONFIG_LOAD_FAILURE, { } );
		dispatchEvent(sfsEvt);
	}
	...
	private function handleHandShake(evt:SFSEvent):Void
	{
		...
		if(obj.isNull(BaseRequest.KEY_ERROR_CODE))
		{
			...
			if(_bitSwarm.isReconnecting)
			{
				...
				*/
				...
				dispatchEvent(SFSEvent.newInst(SFSEvent.CONNECTION_RESUME, { } ));
			}
			else
			{
				/*
				...
				*/
				...
				dispatchEvent(SFSEvent.newInst(SFSEvent.CONNECTION, { success:true } ));
			}
		}
		...
		else
		{
			...
			dispatchEvent(SFSEvent.newInst(SFSEvent.CONNECTION, params));
		}			
	}
	...
	private function handleConnectionProblem(evt:BitSwarmEvent):Void
	{
		...
		if(_bitSwarm.connectionMode==ConnectionMode.SOCKET && _useBlueBox)
		{
			...
			dispatchEvent(SFSEvent.newInst(SFSEvent.CONNECTION_ATTEMPT_HTTP, { } ));
		}
