...
class SFSBuddy implements Buddy {
	...
	/** @private */
	public static function fromSFSArray(arr:Dynamic):Buddy {
		...
		var bVarsData:Dynamic = arr.getSFSArray(3); // variables data array

		for (j in 0...bVarsData.size()) {
			// buddy.setVariable(Dynamic.fromSFSArray(bVarsData.getSFSArray(j)));
		}
	...
	private function get_isOnline():Bool {
		var bv:Dynamic = {}; // getVariable(ReservedBuddyVariables.BV_ONLINE);
	...
	private function get_state():String {
		 ...
		 */
		var bv:Dynamic = {}; // getVariable(ReservedBuddyVariables.BV_STATE);
	...
	private function get_nickName():String {
		var bv:Dynamic = {}; // getVariable(ReservedBuddyVariables.BV_NICKNAME);
		return bv == null ? null : bv.getStringValue();
	}

	/** @inheritDoc */
	public var variables(get, null):Array<Dynamic>;

	private function get_variables():Array<Dynamic> {
	...
	/** @inheritDoc */
	public function getOnlineVariables():Array<Dynamic> {
	...
	/** @private */
	public function setVariable(bVar:Dynamic):Void {
